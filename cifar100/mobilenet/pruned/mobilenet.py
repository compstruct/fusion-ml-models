...
RECOVERY_EPOCHS = 50
WARMUP = 8
EPOCHS = WARMUP + PRUNING_EPOCHS + RECOVERY_EPOCHS
...
setup_gpus(mem_alloc=9.8e3, num_gpu=ACTIVE_GPUS)
...
def get_dataset(dataset_name, shuffle_buff_size=1024, batch_size=BATCH_SIZE, augmented=True):
    ...
    train = train.repeat().shuffle(shuffle_buff_size).map(preprocess, num_parallel_calls=8).batch(batch_size).map(augmentation, num_parallel_calls=8)
...
def prune_depthwise(model, dw_sparsity, pw_sparsity, current_epoch):
    ...
    for l_number, layer in enumerate(model.layers):
        if (isinstance(layer, tf.keras.layers.DepthwiseConv2D) and l_number > 10) or (isinstance(layer, tf.keras.layers.Conv2D) and layer.name in prune_layers):
...
class CustomCheckpoint(tf.keras.callbacks.Callback):
  ...
  """
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current > self.best_accuracy and self.current_epoch > (EPOCHS-RECOVERY_EPOCHS):
            self.best_accuracy = current
            tf.keras.models.save_model(self.model, pruned_model, include_optimizer=False)
...
def get_callbacks():
    ...
    dw_schedule = pruning_schedule(DW_INIT_SPARSITY, DW_FINAL_SPARSITY, EPOCHS-RECOVERY_EPOCHS, begin_step=WARMUP)
    pw_schedule = pruning_schedule(PW_INIT_SPARSITY, PW_FINAL_SPARSITY, EPOCHS-RECOVERY_EPOCHS, begin_step=WARMUP)
    callbacks = []
    #callbacks.append(checkpoint)
    callbacks.append(CustomCheckpoint())
...
def main():
    ...
    with strategy.scope():
        ...
        if training:
            ...
            tf.keras.models.save_model(model, "post_prune_last_epoch.h5", include_optimizer=False)
