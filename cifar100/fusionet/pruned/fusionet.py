...
log_dir_parent = "tb/"
vanilla_model = "vanilla.h5"
pruned_model = "post_prune.h5"



#was trained with 6 GPUs
TOTAL_GPUS = 4
ACTIVE_GPUS = 4
GPU_MEM = 9.8e3
CPUs = 8
...
def prune_depthwise(model, dw_sparsity, pw_sparsity, current_epoch):
    ...
    for l_number, layer in enumerate(model.layers):
        if (isinstance(layer, tf.keras.layers.DepthwiseConv2D) and l_number > 20) or (isinstance(layer, tf.keras.layers.Conv2D) and layer.name in prune_layers):
...
def main():
    ...
    with strategy.scope():
        ...
        if training:
            ...
            #tf.keras.models.save_model(model, pruned_model, include_optimizer=False)
