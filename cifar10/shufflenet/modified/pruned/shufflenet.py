...
def prune_depthwise(model, dw_sparsity, pw_sparsity, current_epoch):
    ...
    for l_number, layer in enumerate(model.layers):
        if (isinstance(layer, tf.keras.layers.DepthwiseConv2D) and l_number > 16) or (isinstance(layer, tf.keras.layers.Conv2D) and layer.name in prune_layers):
...
def main():
    ...
    with strategy.scope():
        ...
        if training:
            ...
            tf.keras.models.save_model(model, "post_prune.h5", include_optimizer=False)
