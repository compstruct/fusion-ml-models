...
setup_gpus(mem_alloc=9e3, num_gpu=ACTIVE_GPUS)
...
def ShuffleUnit(input, in_ch, out_ch, groups=3, expansion=4, stride=1, pix=32, decay=WDECAY, residual=True):
    midd_ch = int(in_ch * expansion)


    pw2 = group_conv(input, in_ch=in_ch, out_ch=midd_ch, groups=groups, decay=decay)
    ...
    if residual:
        pw1 = group_conv(pw2_shuffled, in_ch=midd_ch, out_ch=out_ch if stride==1 and in_ch==out_ch else out_ch-in_ch, groups=groups, decay=decay)
    else:
        pw1 = group_conv(pw2_shuffled, in_ch=midd_ch, out_ch=out_ch, groups=groups, decay=decay)
...
def build_model():
    ...
    Conv1 = tf.keras.layers.Conv2D(filters=24, kernel_size=(3,3), padding='same', use_bias=False, kernel_initializer="he_normal", kernel_regularizer=regularizers.l2(4e-5))(inp)
    Conv1_bn = tf.keras.layers.BatchNormalization()(Conv1)
    Conv1_rl = tf.keras.layers.ReLU()(Conv1_bn)

    pw_init = group_conv(Conv1_rl, in_ch=24, out_ch=24, groups=3, decay=4e-5)
    ...
    block1 = ShuffleUnit(input=dw_init_bn, in_ch=24, out_ch=18, pix=32, expansion=107.25, decay=2e-4, residual=False)
    block2 = ShuffleUnit(input=block1, in_ch=18, out_ch=18, pix=32, expansion=143, decay=2e-4)

    block3 = ShuffleUnit(input=block2, in_ch=18, out_ch=24, pix=32, expansion=143, stride=2)
    block4 = ShuffleUnit(input=block3, in_ch=24, out_ch=24, pix=16, expansion=107.25)
    block5 = ShuffleUnit(input=block4, in_ch=24, out_ch=24, pix=16, expansion=107.25)
    block6 = ShuffleUnit(input=block5, in_ch=24, out_ch=24, pix=16, expansion=107.25)

    block7 = ShuffleUnit(input=block6, in_ch=24, out_ch=42, pix=16, expansion=107.25, stride=2)
    block8 = ShuffleUnit(input=block7, in_ch=42, out_ch=42, pix=8, expansion=62)
    block9 = ShuffleUnit(input=block8, in_ch=42, out_ch=42, pix=8, expansion=62)

    block10 = ShuffleUnit(input=block9, in_ch=42, out_ch=42, pix=8, expansion=62)
    block11 = ShuffleUnit(input=block10, in_ch=42, out_ch=42, pix=8, expansion=62)
    block12 = ShuffleUnit(input=block11, in_ch=42, out_ch=42, pix=8, expansion=62)

    block13 = ShuffleUnit(input=block12, in_ch=42, out_ch=42, pix=8, expansion=62)
    block14 = ShuffleUnit(input=block13, in_ch=42, out_ch=42, pix=8, expansion=62)
    block15 = ShuffleUnit(input=block14, in_ch=42, out_ch=84, pix=8, expansion=62)



    pw_final = group_conv(block15, in_ch=84, out_ch=960, groups=3)
    pw_final_bn = tf.keras.layers.BatchNormalization()(pw_final)
