...
prune_layers =[]
for i in range(2,48):
    if i not in [8, 9, 12, 13, 16, 17, 28, 29, 32, 33, 36, 37, 40, 41, 44, 45]:
...
def get_callbacks():
    ...
    dw_schedule = pruning_schedule(DW_INIT_SPARSITY, DW_FINAL_SPARSITY, EPOCHS-RECOVERY_EPOCHS, begin_step=6)
    pw_schedule = pruning_schedule(PW_INIT_SPARSITY, PW_FINAL_SPARSITY, EPOCHS-RECOVERY_EPOCHS, begin_step=6)
    callbacks = []
