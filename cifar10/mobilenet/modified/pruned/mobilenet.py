...
log_dir_parent = "tb/"
pruned_model = "./../vanilla/vanilla.h5"
pred_model = "post_prune.h5"
...
def InvertedBottleneck(input, in_ch, out_ch, expansion=6, stride=1, l2=WDECAY, layer_index=0):
    ...
    decompress = tf.keras.layers.Conv2D(filters=midd_ch, kernel_size=(1,1), padding='same', use_bias=False, kernel_initializer=tf.constant_initializer(layers[i].get_weights()[0]), kernel_regularizer=regularizers.l2(l2))(input)
    i+=1
    decompress_bn = tf.keras.layers.BatchNormalization(beta_initializer=tf.constant_initializer(layers[i].get_weights()[1]), gamma_initializer=tf.constant_initializer(layers[i].get_weights()[0]),\
                         moving_mean_initializer=tf.constant_initializer(layers[i].get_weights()[2]), moving_variance_initializer=tf.constant_initializer(layers[i].get_weights()[3]))(decompress)
    decompress_relu = tf.keras.layers.ReLU()(decompress_bn)
    i+=2

    convdw = tf.keras.layers.DepthwiseConv2D(kernel_size=(3,3), padding='same', strides = stride, use_bias=False, depthwise_initializer=tf.constant_initializer(layers[i].get_weights()[0]))(decompress_relu)
    i += 1
    convdw_bn = tf.keras.layers.BatchNormalization(beta_initializer=tf.constant_initializer(layers[i].get_weights()[1]), gamma_initializer=tf.constant_initializer(layers[i].get_weights()[0]),\
                         moving_mean_initializer=tf.constant_initializer(layers[i].get_weights()[2]), moving_variance_initializer=tf.constant_initializer(layers[i].get_weights()[3]))(convdw)
    convdw_relu = tf.keras.layers.ReLU()(convdw_bn)
    i+=2

    compress = tf.keras.layers.Conv2D(filters=out_ch, kernel_size=(1,1), padding='same', use_bias=False, kernel_initializer=tf.constant_initializer(layers[i].get_weights()[0]), kernel_regularizer=regularizers.l2(l2))(convdw_relu)
    i+=1
...
def build_model():
    ...
    block0 , i= InvertedBottleneck(input=Conv1_rl, in_ch=32, out_ch=16, expansion=1, layer_index=i)
...
def prune_depthwise(model, sparsity, current_epoch):
    global glb_mask
    for l_number, layer in enumerate(model.layers):
        if isinstance(layer, tf.keras.layers.DepthwiseConv2D) and l_number > 9:
...
class PruneDepthwise(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        prune_depthwise(self.model, self.schedule[self.epoch_number], self.epoch_number)
...
#Pointwise pruning schedule
def apply_pruning(layer):
    prune_layers =[]
    for i in range(3,31):
        prune_layers.append("conv2d_"+str(i))
    pruning_params = {
                     'pruning_schedule': tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=PW_INIT_SPARSITY,
                     final_sparsity=PW_FINAL_SPARSITY, begin_step=0, end_step=np.ceil(50000 / BATCH_SIZE).astype(np.int32) * (EPOCHS - RECOVERY_EPOCHS))}
...
def get_callbacks():
    ...
    callbacks.append( PruneDepthwise( pruning_schedule(DW_INIT_SPARSITY, DW_FINAL_SPARSITY, EPOCHS-RECOVERY_EPOCHS) ) )
...
def main():
    ...
    with strategy.scope():
        if not pre_trained:
            ...
            model = tf.keras.models.clone_model(vanilla_model, clone_function=apply_pruning,)
        ...
        if training:
            ...
            tf.keras.models.save_model(model_for_export, pred_model, include_optimizer=False)
        else:
            #model.summary()
