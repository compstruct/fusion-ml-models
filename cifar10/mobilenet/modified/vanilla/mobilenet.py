...
#####################################################################################################
def InvertedBottleneck(input, in_ch, out_ch, expansion=6, stride=1, l2=WDECAY):
    midd_ch = in_ch * expansion
    decompress = tf.keras.layers.Conv2D(filters=midd_ch, kernel_size=(1,1), padding='same', use_bias=False, kernel_initializer="he_normal", kernel_regularizer=regularizers.l2(l2))(input)
    ...
    compress = tf.keras.layers.Conv2D(filters=out_ch, kernel_size=(1,1), padding='same', use_bias=False, kernel_initializer="he_normal", kernel_regularizer=regularizers.l2(l2))(convdw_rl)
...
def build_model():
    ...
    block1 = InvertedBottleneck(input=block0, in_ch=16, out_ch=24, expansion=54, l2=2e-4)
    block2 = InvertedBottleneck(input=block1, in_ch=24, out_ch=24, expansion=36, l2=2e-4)
    block3 = InvertedBottleneck(input=block2, in_ch=24, out_ch=24, expansion=36, l2=2e-4)

    block4 = InvertedBottleneck(input=block3, in_ch=24, out_ch=32, expansion=36, l2=2e-4, stride=2)
    block5 = InvertedBottleneck(input=block4, in_ch=32, out_ch=32, expansion=27, l2=2e-4)
    block6 = InvertedBottleneck(input=block5, in_ch=32, out_ch=32, expansion=27, l2=2e-4)
    block7 = InvertedBottleneck(input=block6, in_ch=32, out_ch=32, expansion=27, l2=2e-4)

    block8 = InvertedBottleneck(input=block7, in_ch=32, out_ch=40, expansion=27, l2=1e-4, stride=2)
    block9 = InvertedBottleneck(input=block8, in_ch=40, out_ch=40, expansion=21.6, l2=1e-4)
    block10 = InvertedBottleneck(input=block9, in_ch=40, out_ch=40, expansion=21.6, l2=1e-4)
    block11 = InvertedBottleneck(input=block10, in_ch=40, out_ch=40, expansion=21.6, l2=1e-4)

    block12 = InvertedBottleneck(input=block11, in_ch=40, out_ch=40, expansion=21.6, l2=1e-4)
    block13 = InvertedBottleneck(input=block12, in_ch=40, out_ch=40, expansion=21.6, l2=1e-4)
    block14 = InvertedBottleneck(input=block13, in_ch=40, out_ch=80, expansion=21.6, l2=1e-4)

    expanded_lastConv = tf.keras.layers.Conv2D(filters=1280, kernel_size=(1,1), padding='same', use_bias=False, kernel_initializer="he_normal", kernel_regularizer=regularizers.l2(WDECAY))(block14)
